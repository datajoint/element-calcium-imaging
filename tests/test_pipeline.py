...
def test_scan_info(pipeline, scan_info):
    scan = pipeline["scan"]
    expected_scaninfo = {'subject': 'subject1',
...
def test_processing(pipeline, processing):
    ...
    # motion correction
    average_image, correlation_image = imaging.MotionCorrection.Summary.fetch1('average_image', 'correlation_image')
    assert average_image.shape == (512, 512)
    assert correlation_image.shape == (504, 506)

    y_shifts, x_shifts = imaging.MotionCorrection.RigidMotionCorrection.fetch1('y_shifts', 'x_shifts')
    ...
    ftrace = (imaging.Fluorescence.Trace & "fluo_channel = 0" & "mask = 10").fetch1("fluorescence")
    assert len(ftrace) == 3000
    assert ftrace.argmax() == 1031

    # activity
    assert len(imaging.Activity.Trace & "fluo_channel = 0") == 1276
    atrace = (imaging.Activity.Trace & "fluo_channel = 0" & "mask = 20").fetch1("activity_trace")
